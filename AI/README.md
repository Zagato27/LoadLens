...
- Поддержка “пиковой производительности” (peak_performance) в итоговом JSON при наличии соответствующих метрик.
...
- `metrics_source` — источник метрик SUT: `type=prometheus|grafana_proxy`;
...
Для точечных изменений без перезапуска предназначен `settings_runtime.json` (в т.ч. по областям: `per_area[<service>]`).  
