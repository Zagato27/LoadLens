...
Создание отчётов в два клика: сбор метрик, AI‑аналитика по доменам, публикация в Confluence и/или локальный веб‑интерфейс. Поддерживаются Prometheus напрямую и через Grafana‑прокси, а также InfluxDB (напрямую и через Grafana‑прокси) для метрик генератора нагрузки (lt_framework).
