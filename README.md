...
- Главная (дашборд) — краткая сводка по последнему запуску и распределению вердиктов.
...
- Страница отчёта — доменные блоки, итоговый блок, «Итоги инженера».
...
## Безопасность
- Не храните секретные ключи в git. Используйте переменные окружения/секреты и локальные файлы, исключённые из VCS.
- Для Grafana/Prometheus/InfluxDB применяйте отдельные сервисные учётные записи.
